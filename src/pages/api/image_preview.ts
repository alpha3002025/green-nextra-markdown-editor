...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    // Debug Log
    const logPath = path.join(process.cwd(), 'image-debug.log');
    ...
    if (slug === 'home') {
        ...
        if (isFile) {
            ...
            // Logic match with upload.ts:
            // imgDir = path.join(dirname(postDir), 'img', basename(postDir, ext))
            // Here file includes the basename if it was generated by editor.tsx correctly
    ...
    log(`[Preview] Request: slug=${slug}, file=${file}`);
    log(`[Preview] Resolved filePath: ${filePath}`);
    log(`[Preview] Exists: ${fs.existsSync(filePath)}`);
