...
5. **대체 도메인 이름(CNAME)**: `{{도메인명}}` 입력.
6. **사용자 정의 SSL 인증서**: "인증서 요청"을 눌러 ACM에서 `{{도메인명}}`에 대한 인증서(반드시 **us-east-1** 리전)를 발급받고 선택합니다.

### 3단계: Route 53 연결
1. **Route 53** > **호스팅 영역** > `{{도메인명}}` 선택.
2. **레코드 생성** 클릭.
...
커스텀 도메인(`{{도메인명}}`)을 사용할 때는 GitHub Pages와 달리 하위 경로(`/repo-name`)가 필요 없으므로 `site.config.js`를 수정해야 합니다.
...
| 시크릿 이름 | 설명 | 예시 |
|------------|------|------|
| `AWS_S3_BUCKET_NAME` | S3 버킷 이름 | `{{도메인명}}` |
