...
강의 필기를 한다거나, 어떤 책을 읽은 내용들을 정리하거나, 유튜브 경제 방송등을 들을 때 캡쳐 이미지를 드래그앤 드랍으로 추가하거나 Youtube 링크를 추가할 때 Preview 기능이 지원되면 좋다는 생각을 해서 해당 기능 역시 지원됩니다. 제 경우에는 Terraform 강의 필기를 위해 내가 사용하기 위한 목적으로 이 프로젝트를 개설해서 진행하게 되었습니다.<br/>
<br/>

Nextra 와 StackEdit 의 기능이 혼합된 도구를 만들고 싶어서 시작하게 되었는데, 현재 마크다운으로 뭔가를 수정해서 보관하는 서비스는 있지만, 지나치게 공감을 받는다거나 트렌딩에 오른다거나 하는 불안감 때문에 공개하기 꺼려지는 경우도 있는 것 같습니다. 이 프로젝트는 개인적으로 뭔가를 기록하거나 메모하거나 한 내용들을 github repository 를 통해 로컬에서도 확인할 수 있고, vercel, github.io 등을 이용해서도 production 으로 배포해서 강의 노트나 일상생활 속에서 필기한 내용들, 스크랩한 내용 등을 지인들에게 공유할 수 있도록 하기 위해 만들었습니다. 라이선스는 nextra 의 라이선스를 그대로 따르지만, 저 조차도 nextra 의 라이선스가 어떤지는 잘 모릅니다. 그냥 누구든지 자유롭게 사용하시거나 입맛에 맞도록 수정해서 사용하시면 됩니다.<br/>
<br/>

아직 몇몇 부분의 수정이 필요하긴 한데 수정이 필요한 몇몇 소기능은 다음과 같습니다.
- 좌측 Explorer 패널 width 사이즈 조정 기능
- 글쓰기 영역과 미리보기 영역 사이즈 조정 기능
